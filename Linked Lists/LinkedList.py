...
class linked_list:
    ...
    def get(self,index):
        ...
        if not index <= self.__len__():
            raise IndexError(f'Given index: {index} is larger than array size {self.__len__()}')
    ...
    def insert(self, index, data):
        ...
        '''
        if index < 0:
            print("Error: 'Insertion' Index cannot be negative!")
            return
        if index >= self.__len__()(): # Append the node
            cur_node = self.head
